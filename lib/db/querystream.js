    ...
    executeQuery() {
        return new Promise((resolve, reject) => {
            try {
                ...
                var skip = undefined, skipped = 0, take = undefined, taken = 0;
                ...
                request.on('done', (affected) => {
                    if (error != null)
                        ...
                        resolve(new recordset_1.RecordSet(records, affected, (Date.now() - timed), totalRecords >= 0 ? (skipped > 0 ? totalPredicateIterations : totalRecords) : undefined));
                });
                timed = Date.now();
