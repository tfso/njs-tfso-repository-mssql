...
class QueryStream extends query_1.Query {
    ...
    executeQuery() {
        return new Promise((resolve, reject) => {
            try {
                let request, error = null, records = [], totalRecords = -1, totalPredicateIterations = 0, predicate, timed, cancelled = false, completed = false;
                ...
                request.on('done', (result) => {
                    if (error != null)
                        ...
                        resolve(new recordset_1.RecordSet(records, result.rowsAffected.reduce((total, affected) => total += affected, 0), (Date.now() - timed), skip != null ? totalPredicateIterations : (totalRecords >= 0 ? totalRecords : undefined)));
                });
                timed = Date.now();
